...
func TestPersonalTokens(t *testing.T) {
	...
	made, err := svc.CreatePersonalToken(ada, connect.NewRequest(&authv1.CreatePersonalTokenRequest{
		Name: "backup script", Permissions: readOnly, Limited: true, SpaceIds: []string{spaceID}, ExpiresInDays: 90,
	}))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !id.Credential.Reaches(spaceID, "") || !id.Credential.Covers(authctx.MessagesRead) || id.Credential.Covers(authctx.MessagesPost) {
	...
	for name, req := range map[string]*authv1.CreatePersonalTokenRequest{
		...
		"limited without spaces": {Name: "x", Permissions: readOnly, Limited: true, ExpiresInDays: 30},
		"spaces but unlimited":   {Name: "x", Permissions: readOnly, SpaceIds: []string{spaceID}, ExpiresInDays: 30},
		"unknown space":          {Name: "x", Permissions: readOnly, Limited: true, SpaceIds: []string{uuid.NewString()}, ExpiresInDays: 30},
		"over a year":            {Name: "x", Permissions: readOnly, ExpiresInDays: 400},
		"limited with a DM grant": {Name: "x", Permissions: []accessv1.Permission{accessv1.Permission_PERMISSION_DMS_READ},
			Limited: true, SpaceIds: []string{spaceID}, ExpiresInDays: 30},
