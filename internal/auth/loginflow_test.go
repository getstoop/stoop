...
func TestSocialFlowFailures(t *testing.T) {
	...
	// The provider-made account is the first, so the admin and owner; a
	// second admin who owns the server lets the guard allow its
	// deactivation.
	...
	if _, err := svc.TransferOwnership(context.Background(), "", backup.Msg.User.Id); err != nil {
		t.Fatal(err)
	}
