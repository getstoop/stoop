...
- **The big hangout platforms** set the UX benchmark for communities, but
  they are closed: your community's entire history lives on their servers,
  subject to their moderation, monetization, and continuity decisions.
- **Workplace chat** is built for companies — per-workspace identities,
  per-seat pricing, message history held hostage on free tiers.
...
The unserved middle: *a friend group or small community that wants that
hangout experience — spaces, channels, instant messaging, low-latency voice — with
...
- **One host, many communities.** The technical friend hosts once; everyone
  ...
  rather than one identity per community.
...
and per-channel mutes, presence, message editing, replies, reactions,
Markdown, link previews, file uploads (avatars, space icons, attachments),
direct messages and group conversations, message search, pinned messages,
voice channels on LiveKit, webhooks and bots, twenty-seven client-side
themes, a desktop app, and the space and server admin tooling — all from a
...
carry voice audio. The roadmap from here is tracked in GitHub issues as it
becomes concrete.
