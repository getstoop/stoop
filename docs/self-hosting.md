...
Fetch the upgrade script once, then run it from the install directory
whenever a release is out:
...
It fetches the newest release's compose file, shows what that release's
migrations will do to the database and which releases can still start
against it afterwards, lists settings the release's `env.example` has that
your `.env` does not, and asks. Then it backs up the database and the
uploads into `backups/`, keeps the old compose file as
`docker-compose.yml.prev`, puts the new one in place and starts it. If the
new release does not come up healthy it prints the log and the way back.
`sh stoop-upgrade.sh --plan` stops after showing; `--to 0.4.0` picks a
release; `--yes` skips the question. It keeps itself current: a
successful upgrade replaces the script with the new release's copy.

Release notes say when the LiveKit or Postgres pin moves. Moving to a new
Postgres major is the one thing the script refuses to do; see
[Supported Postgres and LiveKit versions](#supported-postgres-and-livekit-versions).

**Going back.** `sh stoop-upgrade.sh rollback` puts the previous compose
file back and restarts. Each release keeps its schema readable by the
release before it, so this needs no restore, unless the upgrade ran a
contract migration: the script says so before it upgrades, and `rollback`
refuses afterwards and points at the backup it took
...
**By hand.** The script only runs compose commands you can run yourself.
...
the backup directory (the upgrade script names its `backups/<date>-<from>-to-<to>`):
