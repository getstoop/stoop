...
check(
  await waitFor(
    async () =>
      (await B.$('input[placeholder="From the person who invited you"]')) !==
      null,
  ),
...
check(
  await waitFor(async () => {
    ...
    return (
      field !== null &&
      (await B.evaluate((e) => e.readOnly && e.value.length === 10, field))
    );
  }),
...
check(
  await waitFor(
    async () =>
      ...
      (await D.$eval("#root", (e) => e.innerText).catch(() => "")).includes(
...
check(
  await waitFor(
    async () =>
      (await A.$eval(
        'select[name="registration-policy"]',
        (e) => e.value,
      ).catch(() => "")) === "3",
  ),
